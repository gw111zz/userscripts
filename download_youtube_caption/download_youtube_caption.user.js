...
function load_caption( selector ) {
	...
	GM_xmlhttpRequest({
		method: 'GET',
		url: 'http://video.google.com/timedtext?hl=' + caption.lang_code + '&lang=' + caption.lang_code + '&name=' + caption.name + '&v=' + unsafeWindow.pageVideoId,
		onload:function(xhr) {
			if ( xhr.responseText !== "" ) {
				...
				Array.forEach(captions, function( caption, i ) {
					...
					if ( caption.getAttribute('dur') )
						...
						lookback[2] = parseInt( unsafeWindow.swfArgs.length_seconds, 10 );
					}
...
function load_captions( select ) {
	GM_xmlhttpRequest({
		method: 'GET',
		url: 'http://video.google.com/timedtext?hl=en&v=' + unsafeWindow.pageVideoId + '&type=list',
		onload: function( xhr ) {
			if ( xhr.responseText === "" ) {
...
(function() {
	var div = document.createElement('div'),
		...
		tabs = document.getElementById('watch-actions-area').getElementsByTagName('div')[0],
		last_element = tabs.getElementsByTagName('div');
	...
	tabs.insertBefore( div, last_element );
